...
/**
 * Drives a single decision: it calls the adapter without blocking, retries transient failures on a
 * scheduler (never a sleeping I/O thread), then enriches the answers, resolves cost and assembles the
 * out-of-band attributes. Failover across fallback adapters is layered on in a later milestone; this
 * milestone evaluates the primary adapter only.
 */
public final class DecisionEngine {
  ...
  public CompletableFuture<DecisionOutcome> evaluate(JevConnection connection, DecisionRequest request,
    ...
    return attempt(adapter, request, 1, attempts)
        .thenApply(response -> {
  ...
  private CompletableFuture<DecisionResponse> attempt(ProviderAdapter adapter, DecisionRequest request,
                                                      int attemptNo, AtomicInteger attempts) {
  ...
  private CompletableFuture<DecisionResponse> onError(ProviderAdapter adapter, DecisionRequest request,
                                                      int attemptNo, AtomicInteger attempts, Throwable error) {
  ...
  private DecisionOutcome assemble(ProviderAdapter adapter, DecisionRequest request, DecisionContext context,
    ...
    if (response.providerReportedCost() != null) {
      ...
      cost = context.pricePerMillionInputTokens()
          .multiply(new BigDecimal(response.inputTokens()))
    ...
    DecisionAttributes attributes = DecisionAttributes.builder()
        .provider(adapter.routeName())
        .requestedModel(response.requestedModel())
        .model(response.model())
        .usage(new TokenUsage(response.inputTokens(), response.outputTokens()))
        ...
        .rawResponse(context.includeRawResponse() ? response.rawBody() : null)
        .build();
